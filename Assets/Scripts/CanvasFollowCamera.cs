...
{
    // Référence é la caméra principale
    public Camera mainCamera;

    // Distance de l'UI par rapport é la caméra
    public float distanceFromCamera = 2f;

    // Angle d'orientation de l'UI par rapport é la caméra (facultatif)
    public Vector3 rotationOffset = Vector3.zero;

    // ^Distance de l'UI par rapport é la caméra (facultatif)
    public Vector3 postionOffset = Vector3.zero;


    void Update()
    {
        // Si la caméra principale n'est pas assignée, on récupére la caméra principale par défaut
        if (mainCamera == null)
        {
        ...
        if (mainCamera != null)
        {
            ...
            // Optionnel : Ajuster la rotation pour toujours faire face é la caméra
